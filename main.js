...
var DEFAULT_SETTINGS = {
  ...
  default_emoji: "\u{1F4E3}",
  saveToNote: false,
  logNotePath: null,
  alsoLogToDaily: false
};
var StatusPosterPlugin = class extends import_obsidian.Plugin {
  constructor() {
    super(...arguments);
  ...
  async handleStatusPost(status, skipMastodon) {
    var _a;
    ...
    try {
      ...
      new import_obsidian.Notice("\u{1F389} Published!");
      ...
      if (this.settings.alsoLogToDaily && this.dailyPluginAvailable) {
        const daily = (_a = (0, import_obsidian_daily_notes_interface.getDailyNote)((0, import_moment.default)(), (0, import_obsidian_daily_notes_interface.getAllDailyNotes)())) != null ? _a : await (0, import_obsidian_daily_notes_interface.createDailyNote)((0, import_moment.default)());
      ...
      const fallbackPath = `Failed Status - ${(0, import_moment.default)().format("YYYY-MM-DD HH-mm")}`;
      await this.app.vault.create(fallbackPath + ".md", `Failed to post:

${status}`);
    }
  }
  async appendToNote(notePath, status, url) {
...
- **${timestamp}**: [${status}](${url || "#"})`;
  ...
  getDataToPost(text, skipMastodon) {
...
var StatusPostModal = class extends import_obsidian.Modal {
  ...
  onOpen() {
    ...
    new import_obsidian.ButtonComponent(contentEl).setButtonText("Post").setCta().onClick(() => this.submit());
  }
  submit() {
    const text = this.statusText.trim();
    if (!text) {
      new import_obsidian.Notice("Please enter a status.");
      return;
    }
...
var StatusPosterSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    new import_obsidian.Setting(containerEl).setName("Username").setDesc("Your omg.lol username").addText(
      (text) => text.setValue(this.plugin.settings.username).onChange(async (value) => {
        this.plugin.settings.username = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("API Token").setDesc("Your omg.lol API token").addText(
      (text) => text.setValue(this.plugin.settings.token).onChange(async (value) => {
        this.plugin.settings.token = value;
        await this.plugin.saveSettings();
      }).inputEl.type = "password"
    );
    new import_obsidian.Setting(containerEl).setName("Default Emoji").setDesc("Used if no emoji is provided at the start of your status").addText(
      (text) => text.setValue(this.plugin.settings.default_emoji).onChange(async (value) => {
        this.plugin.settings.default_emoji = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Save to Daily Note").addToggle(
      (toggle) => {
        var _a;
        return toggle.setValue((_a = this.plugin.settings.alsoLogToDaily) != null ? _a : false).onChange(async (value) => {
          this.plugin.settings.alsoLogToDaily = value;
          await this.plugin.saveSettings();
        });
      }
    );
    new import_obsidian.Setting(containerEl).setName("Save to Custom Note").addToggle(
      (toggle) => {
        var _a;
        return toggle.setValue((_a = this.plugin.settings.saveToNote) != null ? _a : false).onChange(async (value) => {
          this.plugin.settings.saveToNote = value;
          await this.plugin.saveSettings();
        });
      }
    );
    new import_obsidian.Setting(containerEl).setName("Log Note Path").setDesc("Example: logs/status-log").addText(
      (text) => text.setValue(this.plugin.settings.logNotePath || "").onChange(async (value) => {
