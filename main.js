...
var StatusLolPlugin = class extends import_obsidian.Plugin {
  ...
  async saveStatusToLogNote(status, url) {
    ...
    const timestamp = import_obsidian.moment.default().format("YYYY-MM-DD HH:mm");
    const content = `
- **${timestamp}**: [${status}](${url || "#"})`;
  ...
  async saveStatusToDailyNote(status, url) {
    const daily = (0, import_obsidian_daily_notes_interface.getDailyNote)(import_obsidian.moment.default(), (0, import_obsidian_daily_notes_interface.getAllDailyNotes)());
    const note = daily != null ? daily : await (0, import_obsidian_daily_notes_interface.createDailyNote)(import_obsidian.moment.default());
    const timestamp = import_obsidian.moment.default().format("HH:mm");
    const content = `
- **${timestamp}**: [${status}](${url || "#"})`;
    await this.app.vault.append(note, content);
  }
  async saveStatusToFallbackNote(status) {
    const filename = `Failed Status - ${import_obsidian.moment.default().format("YYYY-MM-DD HH-mm")}.md`;
